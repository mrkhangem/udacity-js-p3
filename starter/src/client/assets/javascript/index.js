...
async function handleCreateRace() {
  ...
  // render starting UI
  if (!race_id || !track_id) {
    renderAt("#error", "Pls choose track and racer");
    return;
  }
  try {
    const racer = await createRace(player_id, track_id);
    store = Object.assign(store, { race_id: racer.ID - 1 });
    renderAt("#race", renderRaceStartView(racer.Track));
  } catch (err) {
    renderAt(
      "#error",
  ...
  try {
    ...
    renderAt("#error", `Something went wrong: ${err.message}`);
  }
}
...
function accelerate(id) {
  ...
  return fetch(`${SERVER}/api/races/${id}/accelerate`, {
    ...
    throw new Error(`Problem with accelerate request::${err}`);
  });
}
